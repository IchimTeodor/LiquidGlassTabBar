...
struct ShrinkingTabBarTests {
    ...
    @Test func titlesStaySingleLineWhenShrunk() {
        ...
        // "Favorites" is the longest of the four titles and the one that
        // wrapped; the set is spelled out here rather than pulled from the
        // demo app so the package's tests stand alone.
        let bar = ShrinkingTabBar(items: [
